...
{
	[ExportMethod]
    public void CreateUser(string username, string password, string locale, string rfidId, out NodeId result)
    {
		result = NodeId.Empty;
		...
		result = GenerateUser(username, password, locale, rfidId);
    }

    private NodeId GenerateUser(string username, string password, string locale, string rfidId)
    {
		var users = GetUsers();
		if (users == null)
		{
		...
		foreach (var child in users.Children.OfType<RFID_User>())
		{
		...
		var user = InformationModel.MakeObject<RFID_User>(username);
