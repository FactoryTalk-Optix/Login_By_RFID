...
{
	[ExportMethod]
	public void SaveUser(string username, string password, string locale, string rfidId, out NodeId result)
	{
        result = NodeId.Empty;
        ...
        result = ApplyUser(username, password, locale, rfidId);
	}

	private NodeId ApplyUser(string username, string password, string locale, string rfidId)
	{
		var users = GetUsers();
		if (users == null)
		{
